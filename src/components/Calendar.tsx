...
export const Calendar: FC<CalendarProps> = ({selectedDate: date, selectDate}) => {

	const [mode, setMode] = useState<'days' | 'monthes'>('days')
	const [selectedDate, setSelectedDate] = useState(createDate(date)) 
	const [shownMonth, setShownMonth] = useState(createMonth(new Date(selectedDate.year, selectedDate.monthIndex)))
	...
	const onClickArrow = (direction: 'next' | 'prev') => {
		if (mode ==='days' ) {
			const monthIndex = direction === 'next' ? shownMonth.monthIndex + 1: shownMonth.monthIndex - 1
						...
						<>
						 ...
						 <div className="calendar__days">
							{shownDayList.map((dayItem, idx) => {
								const isSelectedDay = checkDateIsEqual(dayItem.date, selectedDate.date)
								const isAdditionalDay = dayItem.monthIndex !== shownMonth.monthIndex
								...
								return (
									<div 
										key={idx} 
										onClick={() => {
											if (isAdditionalDay) setShownMonth(createMonth(new Date(dayItem.year, dayItem.monthIndex)))
					...
					{mode === 'monthes' && (
						<div className="calendar__pick__item__container">
							{monthNameList.map((monthItem, idx)=>{
								const isSelectedMonth = monthItem.monthIndex === selectedDate.monthIndex && monthItem.year === selectedDate.year
